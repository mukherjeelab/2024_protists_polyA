...
gmur_ref <- fread(here('data/annotation/gmur_reference_annot.tsv.gz'))

gmur_ref_3utr <- gmur_ref %>%
...
gmur_ref_5utr <- gmur_ref %>%
  dplyr::select(c('seq_id','primary_tag','start','end','gene_id')) %>%
  filter(primary_tag == 'five_prime_utr') %>%
  dplyr::rename(c('utr5_annot_start'='start','utr5_annot_end'='end','GeneID'='gene_id','Chr'='seq_id'))%>%
  dplyr::select(-c('primary_tag'))

fc_gmur <- fread(here('data/featureCounts/gmur_feature_counts_cleaned.tsv.gz')) %>%
  left_join(.,gmur_ref_3utr) %>%
  left_join(.,gmur_ref_5utr) %>%
...
annot_gmur2 <- fread(here('data/featureCounts/gmur2_feature_counts_annot_filtered.tsv.gz')) %>%
  filter(V2 == 'Assigned' & V1 %in% polyA_gmur2$readname) %>%
  distinct(.,V1, .keep_all = T) %>%
  dplyr::select(c(1,4)) %>%
  set_colnames(c('readName','GeneID')) %>%
  left_join(.,fc_gmur2)
compare_gmur2 <- fread(here('data/alignment_coords/gmur-2.sorted.bed.gz')) %>%
  distinct(V4, .keep_all = T) %>%
  dplyr::select(-c(5)) %>%
  set_colnames(c('Chr','readStart','readEnd','readName','Strand')) %>% 
  left_join(annot_gmur2,.) %>%
  dplyr::select(c(1,2,4,5,9,6,10,7,8)) %>%
  na.omit() %>%
  filter(if_else(Strand == '+', readEnd > End & readStart < End, readStart < Start & readEnd > Start)) %>%
  mutate(dataset = 'gmur-2')

#combining data and calculating isoforms frequencies
gmur_all <- rbind(compare_gmur1, compare_gmur2) %>%
  mutate(Start = as.integer(Start), End = as.integer(End)) %>%
  mutate(utr3_length = if_else(Strand == '+', (readEnd-End)+1, (Start-readStart)+1)) %>%
  arrange(GeneID)

gmur_groupped <- gmur_all %>%
...
gmur_iso_freq_tmp <- gmur_groupped %>% 
  filter(counts >= 10) %>%
  group_by(GeneID) %>%
...
group_iso_multiples_1 <- gmur_iso_freq_tmp %>%
  filter(category=='multiples' & cl_site_n == 1)

group_iso_multiples_filtered <- gmur_iso_freq_tmp %>%
  filter(category=='multiples' & cl_site_n > 1) %>%
  group_by(GeneID) %>%
...
gmur_iso_freq <- rbind(subset(gmur_iso_freq_tmp, !category =='multiples'), group_iso_multiples_1, group_iso_multiples_filtered) %>%
  group_by(GeneID) %>%
  arrange(GeneID) %>%
  mutate(cl_site_n = 1:n(), sum_iso = sum(iso_freq), mean_site = mean(cl_site_n)) %>%
  mutate(category = case_when(mean_site == 1 ~ 'singles',
...
gmur_3UTR_coord_full_transcr <- gmur_iso_freq %>%
  dplyr::select(c('GeneID','utr3_length')) %>%
  left_join(., subset(fc_gmur, GeneID %in% .$GeneID, select = c(GeneID:Strand))) %>%
...
gmur_5UTR_coord_full_transcr <- gmur_all %>%
...
full_transcr_coord_4gtf <- gmur_3UTR_coord_full_transcr %>%
  left_join(., gmur_5UTR_coord_full_transcr, by = c('GeneID')) %>%
  ...
  mutate(source = 'nanopore', feature = 'transcript', score = '.', frame = '.', tr_counts = 1:n(),
         ...
         description = paste0(' description "',gmur_ref$description[match(GeneID, gmur_ref$gene_id)],'";'),
...
utr3_coord_4gtf_pos <- gmur_3UTR_coord %>%
  filter(Strand == '+') %>%
...
utr3_coord_4gtf_neg <- gmur_3UTR_coord %>%
  filter(Strand == '-') %>%
  dplyr::select(-c('score')) %>%
  set_colnames(c('Chr.x','Start','End','GeneID','Strand.x')) %>%
...
utr5_coord_4gtf_pos <- gmur_5UTR_coord %>%
  filter(Strand == '+') %>%
  dplyr::select(-c('score')) %>%
  set_colnames(c('Chr.x','Start','End','GeneID','Strand.x')) %>%
  left_join(.,subset(full_transcr_coord_4gtf, select = -c(End))) %>%
...
utr5_coord_4gtf_neg <- gmur_5UTR_coord %>%
  filter(Strand == '-') %>%
  dplyr::select(-c('score')) %>%
  set_colnames(c('Chr.x','Start','End','GeneID','Strand.x')) %>%
  left_join(.,subset(full_transcr_coord_4gtf, select = -c(Start))) %>%
...
cds_coord_4gtf <- full_transcr_coord_4gtf %>%
  ...
  left_join(.,subset(gmur_ref, primary_tag == 'CDS', select = c(1,4,5,8,10)), by = c('GeneID'='gene_id', 'Chr.x'='seq_id')) %>%
...
exon_coord_4gtf <- full_transcr_coord_4gtf %>%
  dplyr::select(-c('Start','End','frame')) %>%
  left_join(.,subset(gmur_ref, primary_tag == 'exon', select = c(1,4,5,8,10)), by = c('GeneID'='gene_id', 'Chr.x'='seq_id')) %>%
  dplyr::select(c(1,2,3,4,5,9,10,6,7,11,8)) %>%
  mutate(feature = 'exon') %>%
  dplyr::rename(c('Start' = 'start','End'='end')) %>%
  na.omit()
...
gmur_gtf_new <- rbind(full_transcr_coord_4gtf, utr3_coord_4gtf_pos,utr3_coord_4gtf_neg, utr5_coord_4gtf_pos,utr5_coord_4gtf_neg,cds_coord_4gtf,exon_coord_4gtf,gene_coord_4gtf) %>%
  arrange(Chr.x,Start) %>%
  ungroup() %>%
...
utr3_comparison <- bind_rows(utr3_coord_4gtf_pos,utr3_coord_4gtf_neg) %>%
  ...
  left_join(.,gmur_ref_3utr, by = c('GeneID'='GeneID','Chr.x'='Chr')) %>%
  ungroup() %>%
...
if(length(terms_to_filter) > 0) {
  filtered_gtf <- gmur_gtf_new %>%
    filter(!grepl(paste0(terms_to_filter, collapse="|"), V9))
  if(nrow(filtered_gtf > 0)) {
    fwrite(filtered_gtf, here('data/annotation/gmur_nanopore_filtered.gtf'), sep = '\t', col.names = F, quote = F)
    system("gzip data/annotation/gmur_nanopore_filtered.gtf")
  }
} else {
  fwrite(gmur_gtf_new, here('data/annotation/gmur_nanopore.gtf'), sep = '\t', col.names = F, quote = F)
  system("gzip data/annotation/gmur_nanopore.gtf")
}