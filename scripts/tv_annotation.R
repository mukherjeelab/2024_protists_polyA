...
trich_ref <- fread(here('data/annotation/trich_reference_annot.tsv.gz'))

trich_ref_3utr <- trich_ref %>%
...
trich_ref_5utr <- trich_ref %>%
  dplyr::select(c('seq_id','primary_tag','start','end','gene_id')) %>%
  filter(primary_tag == 'five_prime_utr') %>%
  dplyr::rename(c('utr5_annot_start'='start','utr5_annot_end'='end','GeneID'='gene_id','Chr'='seq_id'))%>%
  dplyr::select(-c('primary_tag'))

fc_trich <- fread(here('data/featureCounts/trich_feature_counts_cleaned.tsv.gz')) %>%
  left_join(.,trich_ref_3utr) %>%
  left_join(.,trich_ref_5utr) %>%
...
annot_trich2 <- fread(here('data/featureCounts/trich2_feature_counts_annot_filtered.tsv.gz')) %>%
  filter(V2 == 'Assigned' & V1 %in% polyA_trich2$readname) %>%
  distinct(.,V1, .keep_all = T) %>%
  dplyr::select(c(1,4)) %>%
  set_colnames(c('readName','GeneID')) %>%
  left_join(.,fc_trich2)
compare_trich2 <- fread(here('data/alignment_coords/trich-2.sorted.bed.gz')) %>%
  distinct(V4, .keep_all = T) %>%
  dplyr::select(-c(5)) %>%
  set_colnames(c('Chr','readStart','readEnd','readName','Strand')) %>% 
  left_join(annot_trich2,.) %>%
  dplyr::select(c(1,2,4,5,9,6,10,7,8)) %>%
  na.omit() %>%
  filter(if_else(Strand == '+', readEnd > End & readStart < End, readStart < Start & readEnd > Start)) %>%
  mutate(dataset = 'trich-2')
...
annot_trich3 <- fread(here('data/featureCounts/trich3_feature_counts_annot_filtered.tsv.gz')) %>%
  filter(V2 == 'Assigned' & V1 %in% polyA_trich3$readname) %>%
  distinct(.,V1, .keep_all = T) %>%
  dplyr::select(c(1,4)) %>%
  set_colnames(c('readName','GeneID')) %>%
  left_join(.,fc_trich3)
compare_trich3 <- fread(here('data/alignment_coords/trich-3.sorted.bed.gz')) %>%
  distinct(V4, .keep_all = T) %>%
  dplyr::select(-c(5)) %>%
  set_colnames(c('Chr','readStart','readEnd','readName','Strand')) %>% 
  left_join(annot_trich3,.) %>%
  dplyr::select(c(1,2,4,5,9,6,10,7,8)) %>%
  na.omit() %>%
  filter(if_else(Strand == '+', readEnd > End & readStart < End, readStart < Start & readEnd > Start)) %>%
  mutate(dataset = 'trich-3')

#combining data and calculating isoforms frequencies
trich_all <- rbind(compare_trich1, compare_trich2, compare_trich3) %>%
  mutate(Start = as.integer(Start), End = as.integer(End)) %>%
  mutate(utr3_length = if_else(Strand == '+', (readEnd-End)+1, (Start-readStart)+1)) %>%
  arrange(GeneID)

trich_groupped <- trich_all %>%
...
trich_iso_freq_tmp <- trich_groupped %>% 
  filter(counts >= 10) %>%
  group_by(GeneID) %>%
...
group_iso_multiples_1 <- trich_iso_freq_tmp %>%
  filter(category=='multiples' & cl_site_n == 1)

group_iso_multiples_filtered <- trich_iso_freq_tmp %>%
  filter(category=='multiples' & cl_site_n > 1) %>%
  group_by(GeneID) %>%
...
trich_iso_freq <- rbind(subset(trich_iso_freq_tmp, !category =='multiples'), group_iso_multiples_1, group_iso_multiples_filtered) %>%
  group_by(GeneID) %>%
  arrange(GeneID) %>%
  mutate(cl_site_n = 1:n(), sum_iso = sum(iso_freq), mean_site = mean(cl_site_n)) %>%
  mutate(category = case_when(mean_site == 1 ~ 'singles',
...
trich_3UTR_coord_full_transcr <- trich_iso_freq %>%
  dplyr::select(c('GeneID','utr3_length')) %>%
  left_join(., subset(fc_trich, GeneID %in% .$GeneID, select = c(GeneID:Strand))) %>%
...
trich_3UTR_coord <- trich_iso_freq %>%
  dplyr::select(c('GeneID','utr3_length')) %>%
  left_join(., subset(fc_trich, GeneID %in% .$GeneID, select = c(GeneID:Strand))) %>%
  mutate(Start = as.numeric(Start), End = as.numeric(End)) %>%
  mutate(cl_site = if_else(Strand == '+', End + utr3_length, Start - utr3_length)) %>%
...
trich_5UTR_coord_full_transcr <- trich_all %>%
...
full_transcr_coord_4gtf <- trich_3UTR_coord_full_transcr %>%
  left_join(., trich_5UTR_coord_full_transcr, by = c('GeneID')) %>%
  ...
  mutate(source = 'nanopore', feature = 'transcript', score = '.', frame = '.', tr_counts = 1:n(),
         ...
         description = paste0(' description "',trich_ref$description[match(GeneID, trich_ref$gene_id)],'";'),
...
utr3_coord_4gtf_pos <- trich_3UTR_coord %>%
  filter(Strand == '+') %>%
...
utr3_coord_4gtf_neg <- trich_3UTR_coord %>%
  filter(Strand == '-') %>%
  dplyr::select(-c('score')) %>%
  set_colnames(c('Chr.x','Start','End','GeneID','Strand.x')) %>%
...
utr5_coord_4gtf_pos <- trich_5UTR_coord %>%
  filter(Strand == '+') %>%
  dplyr::select(-c('score')) %>%
  set_colnames(c('Chr.x','Start','End','GeneID','Strand.x')) %>%
  left_join(.,subset(full_transcr_coord_4gtf, select = -c(End))) %>%
...
utr5_coord_4gtf_neg <- trich_5UTR_coord %>%
  filter(Strand == '-') %>%
  dplyr::select(-c('score')) %>%
  set_colnames(c('Chr.x','Start','End','GeneID','Strand.x')) %>%
  left_join(.,subset(full_transcr_coord_4gtf, select = -c(Start))) %>%
...
cds_coord_4gtf <- full_transcr_coord_4gtf %>%
  ...
  left_join(.,subset(trich_ref, primary_tag == 'CDS', select = c(1,4,5,8,10)), by = c('GeneID'='gene_id', 'Chr.x'='seq_id')) %>%
...
exon_coord_4gtf <- full_transcr_coord_4gtf %>%
  dplyr::select(-c('Start','End','frame')) %>%
  left_join(.,subset(trich_ref, primary_tag == 'exon', select = c(1,4,5,8,10)), by = c('GeneID'='gene_id', 'Chr.x'='seq_id')) %>%
  dplyr::select(c(1,2,3,4,5,9,10,6,7,11,8)) %>%
  mutate(feature = 'exon') %>%
  dplyr::rename(c('Start' = 'start','End'='end')) %>%
  na.omit()
...
trich_gtf_new <- rbind(full_transcr_coord_4gtf, utr3_coord_4gtf_pos,utr3_coord_4gtf_neg, utr5_coord_4gtf_pos,utr5_coord_4gtf_neg,cds_coord_4gtf,exon_coord_4gtf,gene_coord_4gtf) %>%
  arrange(Chr.x,Start) %>%
  ungroup() %>%
...
utr3_comparison <- bind_rows(utr3_coord_4gtf_pos,utr3_coord_4gtf_neg) %>%
  ...
  left_join(.,trich_ref_3utr, by = c('GeneID'='GeneID','Chr.x'='Chr')) %>%
  ungroup() %>%
...
if(length(terms_to_filter) > 0) {
  filtered_gtf <- trich_gtf_new %>%
    filter(!grepl(paste0(terms_to_filter, collapse="|"), V9))
  if(nrow(filtered_gtf > 0)) {
    fwrite(filtered_gtf, here('data/annotation/trich_nanopore_filtered.gtf'), sep = '\t', col.names = F, quote = F)
    system("gzip data/annotation/trich_nanopore_filtered.gtf")
  } 
} else {
  fwrite(trich_gtf_new, here('data/annotation/trich_nanopore.gtf'), sep = '\t', col.names = F, quote = F)
  system("gzip data/annotation/trich_nanopore.gtf")
}