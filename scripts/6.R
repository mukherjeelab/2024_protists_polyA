...
pdf(here('./figs/R2_drop_non_giardia.pdf'), width = 10, height = 5)
ggbarplot(r_sq_drop_all, x = 'term', y = 'explained_variance', fill = 'black',
          facet.by = 'dataset', label = TRUE,
          lab.col = "white", lab.pos = "in", lab.hjust = 0.5, lab.vjust = 1) +
  rremove('xlab') +
  ylab('adjusted R^2') +
...
tv_plt <- ggplot(tv_df_full , aes(x = term, y = estimate)) +
  ...
  scale_color_manual(values = c('blue', 'orange', 'gray')) +
  theme(axis.text.x = element_text(angle = 45, hjust=1))
pdf(here('figs/lm_estimates_non_giardia.pdf'), width = 6, height = 12) 
ggarrange(ng_plt, tf_plt, tv_plt, nrow =3)
dev.off()
pdf(here('figs/lm_estimates_non_giardia2.pdf'), width = 6, height = 12) 
ggarrange(ng_plt, tf_plt, tv_plt, nrow =1)
dev.off()
